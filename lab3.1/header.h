...
goods* high_price(int amount, goods* ptr, int* exp_size);                                                     // поиск максимальной цены
...
void free_mem(goods* names_ind, goods* expensive, goods* ptr, int amount);                                    // очистка памяти
goods* create_names_ind(int amount);                                                          // создание массива структур отобранных товаров 
void output(goods* items, int size, const char* header, int output_variant, const char* filename);          // вывод
